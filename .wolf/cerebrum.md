...
- [2026-08-29] **Prose had drifted too literary; Wayne pushed back on it.** "they're too verbose and the writing style is off... more succinct and written in simpler, friendlier language." The failure mode to avoid: 50-word bullets, long comma-chained sentences, and aphorisms as explanation ("A help tray that overstates is worse than no help at all"). Write plainly, one idea per sentence, and let a bullet pay out in its first six words.
- [2026-08-29] **README order: impact first, then the screenshots, then detail.** Wayne's ask. A short "What it does best" bullet list goes ABOVE the screenshot table; everything else lives below it and must not be a wall of text.
...
- [2026-08-31] **Four per-device tables are keyed by `foldName`, not three:** weights, paces, pictures and now the custom exercises. A rename on the exercises page has to move all of the first three, or the number, the photo and the measured pace are orphaned under a name nothing asks about again.
- [2026-08-31] **A `title` attribute does NOT name a button that has text content.** It is a fallback. Two Edit buttons with the same text are indistinguishable to a screen reader and to `getByRole`, so give repeated row actions an `aria-label` that STARTS with the visible word (`aria-label="Edit Leg Press"` over the word "Edit").
- [2026-08-31] **`generateRoutine`'s options bag is the extension point.** `weights`, `rates` and now `extra` all default to empty so the function stays pure and every existing test sees the shipped table alone. Add there rather than changing the signature.
...
- [2026-09-01] **`npx knip` works here and is the fast dead-export sweep.** It found LOADABLE_GROUPS (dead since the exercises page listed every kit), plus needless exports on internal helpers. Ignore its hits on `exercises.shapes.ts` constants (SECTIONS_MIN, SECTION_SIZE): those are GENERATED by scripts/harvest-shapes.test.ts and document corpus facts; deleting them means editing the generator and they return on the next harvest.
- [2026-09-01] **A new BEM modifier class must have a rule before the work is called done.** `exdlg__cand--inline` shipped in JSX with no CSS anywhere; grep new `block__elem--mod` names against the stylesheets. See bug-157.
