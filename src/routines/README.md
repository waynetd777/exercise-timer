...
GENERATED by `npm run harvest` from the sixteen instructor routines, and holds
...
each movement is prescribed, including a seconds-per-rep rate for the fourteen the
instructor has written both ways. `exercises.shapes.ts` holds the nineteen ladder
pyramids he actually writes, used VERBATIM, and the six section themes in the
...
`npm run harvest` runs the scripts in `scripts/` over the sixteen instructor
routines in `__tests__/emails` and rewrites three generated tables. Three traps
...
- **A harvest must not treat its own output as an input.** Comparing against
  ...
  sixteen rows as already known, write none, and silently drop them from the
