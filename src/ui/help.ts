...
export const LIBRARY_HELP: readonly HelpSection[] = [
  {
    ...
    points: [
      ...
      'Search by name; the × clears it. Sort by Recent, Name or Longest.',
...
export const EDITOR_HELP: readonly HelpSection[] = [
  {
    ...
    points: [
      ...
      'Where nothing in the list matches what you typed, it offers to add it as an exercise of your own. Answer its questions and the step goes onto it, ready for a generated routine.',
      'That dialog also takes a weight and a picture, and starts from whatever the step is already carrying. Saving them moves both onto the exercise, so the step stops stating its own and every routine naming it follows.',
  ...
  },
  {
    ...
    points: [
      ...
      'The × in the field empties it again, which is how a step stops overriding and goes back to following the page.',
      'Where that page has no weight for the exercise yet, it asks whether the one you just typed is your weight for it. Add it and every routine naming that exercise follows; keep it here and only this routine changes.',
      ...
      'The same question is asked about a picture the Exercises page has none for: show it everywhere, or on this step alone.',
...
export const EXERCISES_HELP: readonly HelpSection[] = [
  {
    ...
    points: [
      ...
      `All ${EXERCISES.length} the app ships with are listed, plus any you have added, because a press-up has no weight and still has a picture. The weight field only shows on a row with something to weigh.`,
      'Each row says what the exercise works under its name: the body area, push or pull, the station on the multi-gym, the attachment it needs, whether it is worked one side at a time.',
      'The kits are closed when the page opens. Tap a heading to open one; the number beside it says how many are in there, and how many of those you added.',
      'Only one kit stays open at a time, so the heading you want is never a scroll away.',
      'Searching opens all of them, so nothing is hidden behind a heading. The × clears the search and closes them again.',
      'A weight is free text, so a band can be “red” and a dumbbell exercise “5kg each hand”.',
      'The × in a weight field clears it, the same one the search box has. It only appears on a row this page has a weight for.',
    ],
  },
  {
  ...
  },
  {
    ...
    points: [
      ...
      'It takes a weight and a picture at the same time, and Edit opens on the ones the row already shows. Both are what this page holds, so every routine naming the exercise follows them.',
      ...
      'Add is offered in the editor too. Type a name into a work step, and where nothing in the list matches it, the list offers to add it.',
      'Before it adds one, it warns you if something similar is already here. A different spelling of the same movement would otherwise keep a second weight, a second picture and a second pace.',
      'Renaming one carries its weight, picture and measured pace with it, and offers to rename the steps in your routines that still say the old name.',
      'Removing one leaves your routines alone. The steps keep the name as plain text, exactly as a typed name was before.',
    ],
  },
  {
