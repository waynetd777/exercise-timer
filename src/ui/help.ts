...
export const LIBRARY_HELP: readonly HelpSection[] = [
  ...
  },
  {
    ...
    points: [
      ...
      'A generated routine announces the next exercise and its weight before the gap, so you can set the machine up while the minute runs. That announcement is 30 seconds for a multi-gym exercise and 15 for anything else, because a press-up has nothing to rig.',
      ...
      'A multi-gym set asks for 12 reps inside 20 seconds, so the clock paces you and the count is the target. It reads as “12 × Leg Press 65kg” while you run.',
      'A routine of counted reps has no fixed length, so the library says “about 35 min” for one and an exact time for a timed routine. The estimate comes from how long your own routines say an exercise takes: some of them are written both ways, a 30-second Plank one week and 20 × Plank another.',
